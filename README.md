...
For the moment once you push the button **"Change File"**, the method ```on_change_file_clicked()``` is called: it creates a simple random-filled histogram and plots it using the ```plot_data(TObject *pGraph)``` method. As one can see, the method accepts generic **TObject** file as the input, so any type of graphical data can be sent there: **TH1F**, **TGraph**, whatever...
